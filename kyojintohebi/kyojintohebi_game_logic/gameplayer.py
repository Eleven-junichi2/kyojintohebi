...
class GamePlayerList:
    ...
    def __init__(self):
        self.__player_data = {"id": {}}

    def get_player_data(self):
        return self.__player_data

    player_data = property(fget=get_player_data)


    def add_player(self, player_id):
        self.player_data["id"].update({player_id: {
            "name": "",
            "turn_num": player_id,
            "piece": {
                "king": "",
    ...
    def exchange_turn_num(self, p1_id, p2_id):
        p1_turn_num = self.player_data["id"][p1_id]["turn_num"]
        p2_turn_num = self.player_data["id"][p2_id]["turn_num"]
        self.player_data["id"][p1_id]["turn_num"] = p2_turn_num
        self.player_data["id"][p2_id]["turn_num"] = p1_turn_num


    def set_turn_num(self, player_id, turn_num):
        for p_id in self.get_id_list():
            if self.player_data["id"][p_id]["turn_num"] == turn_num:
                self.player_data["id"][p_id]["turn_num"] = self.player_data["id"][player_id]["turn_num"]
                break
        self.player_data["id"][player_id]["turn_num"] = turn_num
    ...
    def get_turn_num_in_player(self):
        ...
        turn_num_in_player = {}
        for p_id in self.get_id_list():
            turn_num_in_player[self.player_data["id"][p_id]["turn_num"]] = p_id
    ...
    def sort_turn_num(self):
        ...
        for p_id, turn_num in zip(self.get_id_list(), turn_num_list):
            self.player_data["id"][p_id]["turn_num"] = turn_num

    def set_name(self, player_id, name):
        self.player_data["id"][player_id]["name"] = name

    def set_piece(self, id, piece_type, piece):
        if not piece_type == "spawn_point":
            self.player_data["id"][id]["piece"][piece_type] = piece
        else:
            raise ValueError("Can't set spawn_point.")

    def set_spawn_point(self, id, point_num, x, y):
        self.player_data["id"][id]["piece"]["spawn_point"][point_num]["x"] = x
        self.player_data["id"][id]["piece"]["spawn_point"][point_num]["y"] = y

    def get_data_type_list(self, id):
        return list(self.player_data["id"][id].keys())

    def get_id_list(self):
        return list(self.player_data["id"].keys())

    def get_spawn_point_num_list(self, id):
        return list(self.player_data["id"][id]["piece"]["spawn_point"].keys())

    def get_name_player_id(self, name):
        for player_id in self.get_id_list():
            if self.player_data["id"][player_id]["name"] == name:
                return player_id
